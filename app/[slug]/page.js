...
export async function generateStaticParams() {
  await dbConnect();
  const apps = await App.find({ isActive: true }).select("slug").lean();
...
export default async function AppPage({ params }) {
  ...
  breadcrumbItems.push({ name: app.name, url: `${SITE_URL}/${app.slug}` });
  ...
  const jsonLd = {
    ...
    "@graph": [softwareApplicationNode, breadcrumbNode],
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto max-w-5xl px-0 pb-2 lg:pb-10">

        {/* ══════════ BREADCRUMB ══════════ */}
        <nav
          aria-label="Breadcrumb"
          className="mt-3 px-2 flex flex-wrap items-center gap-1 text-[11px] font-medium text-slate-400"
        >
          ...
          <span>/</span>
          <span className="truncate text-slate-500" aria-current="page">
            {app.name}
          </span>
        </nav>

        {/* ══════════ H1 TITLE ══════════ */}
        <h1 className="mt-3 px-2 text-xl font-extrabold leading-snug text-slate-900 lg:text-3xl">
          {app.appTitle}
        </h1>
        ...
        <p className="mt-1 px-2 text-xs font-semibold text-slate-400">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <a
              ...
              className="flex h-12 w-full items-center justify-center gap-1.5 rounded-md bg-emerald-500 text-[13px] font-extrabold tracking-wide text-white shadow-md shadow-emerald-200 transition active:scale-[.98] hover:bg-emerald-600"
            >
            ...
            {/* Telegram CTA */}
            <a
              href="https://telegram.dog/+6XcQfvCgkvZmOThl"
              target="_blank"
              rel="noopener noreferrer"
              className="flex h-12 w-full items-center justify-center gap-1.5 rounded-md bg-blue-500 text-[13px] font-extrabold tracking-wide text-white shadow-md shadow-blue-200 transition active:scale-[.98] hover:bg-blue-600"
            >
          ...
          <div className="flex flex-col gap-3">
            {/* Rating */}
            <div className="flex flex-col gap-1 rounded-md border border-slate-100 px-3 py-2.5">
            ...
            {/* Spec table */}
            <div className="rounded-md border border-slate-100 px-3 py-0.5 h-fit">
              {[
        ...
        {relatedApps.length > 0 && (
          <section className="mt-4 border border-slate-200 bg-white p-2">
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 px-4 py-2 backdrop-blur-sm lg:hidden">
        <a
          href={`/api/download/${app.slug}`}
          className="flex h-12 w-full items-center justify-center gap-2 rounded-md bg-emerald-500 text-[14px] font-extrabold text-white shadow-md active:scale-[.98]"
        >
